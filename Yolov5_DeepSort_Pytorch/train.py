...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.num_epochs + 1):
        ...
        if training_step == 3 or epoch == 100:
...
def train(args, model, train_loader, optimizer, epoch, training_step, writer):
    losses = utils_stgat.AverageMeter("Loss", ":.6f")
    progress = utils_stgat.ProgressMeter(
...
def validate(args, model, val_loader, epoch, writer):
    ade = utils_stgat.AverageMeter("ADE", ":.6f")
    fde = utils_stgat.AverageMeter("FDE", ":.6f")
    progress = utils_stgat.ProgressMeter(len(val_loader), [ade, fde], prefix="Test: ")
...
if __name__ == "__main__":
    ...
    utils_stgat.set_logger(os.path.join(args.log_dir, "train.log"))
